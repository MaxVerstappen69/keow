...
$sql = "SELECT employee_id FROM `employee` WHERE em_email = '$id'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
  // Assuming only one row will be fetched
  $row = $result->fetch_assoc();
  $employee_id = $row["employee_id"];
} else {
  // Handle the case where no employee_id is found for the logged-in user
  $employee_id = ""; // Set a default value or handle as needed
}
?>
